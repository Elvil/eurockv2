 ...
 */
package fr.utbm.info.vi51.project.guiOld;
...
public class GUI extends AbstractFrameworkGUI {
	...
	protected void paintDataStructure(Graphics2D g2d, SpatialDataStructure<?> dataStructure) {
		...
		if (dataStructure instanceof QuadTree) {
			QuadTree tree = (QuadTree) dataStructure;
			for(Iterator<QuadTreeNode<ShapedObject>> it = tree.iterator(); it.hasNext() ;)
			{
			//for (QuadTreeNode node : tree) {
				Shape2f<?> bounds = it.next().getBounds();
				Shape shape = mas2screen(bounds);
				g2d.setColor(Color.MAGENTA);
				g2d.draw(shape);
				if (showEntities) {
					g2d.setColor(Color.BLUE);
					for (ShapedObject obj : it.next().getData()) {
